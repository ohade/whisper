/**
 * Process Recording Script (V2)
 * 
 * This script processes an audio recording file through the full flow using the same
 * code path as the main application.
 * 
 * Usage: node process-recording-v2.js --lang=hebrew|english --file=/path/to/file.webm
 */
...
if (!languageArg || !fileArg) {
  ...
  console.error('Usage: node process-recording-v2.js --lang=hebrew|english --file=/path/to/file.webm');
  process.exit(1);
}
...
/**
 * Main function to process a recording
 */
async function main() {
  try {
    console.log('Starting recording processing with V2 implementation...');
